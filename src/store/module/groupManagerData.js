...
const groupManagerData = {
  ...
  mutations: {
    ...
    bindOrderObject(
      state,
      { toolsObject, groupBuyId, isShowGroupBuy, userAccessStatus }
    ) {
  ...
  actions: {
    ...
    setupShareOption(
      ...
      dispatch('getUserInfo', null, { root: true }).then(user => {
        ...
        if (getters.isFromShare) {
          switch (orderStatus) {
            case 1202: //拼团中
              title = `我正在参加《${courseName}》拼团活动,仅差${leavePerson ||
                0}人,快来和我一起拼团吧!`
              break
            default:
              title = courseName
          ...
          if (1202 == orderStatus) {
            ...
            link = `${rootState.url}/#/detail/${
              rootState.columnType
            }/${courseId}?groupBuyId=${groupBuyId}`
          } else {
          ...
          if (1005 == state.userAccessStatus) {
            ...
            link = `${
              rootState.url
            }/#/praise/active/${courseId}/${collectLikeId}?columnType=${
              rootState.columnType
            }`
          } else {
            // link = `${rootState.url}/#/${courseType[rootState.columnType]}${courseId}`
        ...
        let shareData = {
          ...
          desc: '你一定会爱上国学课...',
          imgUrl: `${
            rootState.columnDetail.sharePostUrl
    ...
    initToolsBar({ commit, dispatch }, toolsData) {
      ...
      let isShowGroupBuy
      const personStr =
        groupData.groupBuyPersonCount > 3 ? '六人拼团' : '三人拼团'
      let toolsObject = {
      ...
      switch (userAccessStatus) {
        case -3:
          ...
          if (
            praiseData.collectLikeTemplateId &&
            groupData.groupBuyTemplateId
          ) {
    ...
    async getGroupBuyDetail({ commit, dispatch }, groupBuyId) {
      ...
      let isFullStaff =
        personNum == 0 || result.personCount == result.userList.length
      //4.当前用户是否完成支付
      ...
      result.userList.forEach(currentValue => {
        if (currentValue.status == 2601 && isAllPay) {
          isAllPay = 0
        }
        if (currentValue.id == currentUserId) {
          ;(isGroupCurrent = 1), (currUserStatus = currentValue.status)
        }
      })
      //8.计算倒计时
      const countTime =
    ...
    async updateFatherData({ dispatch, state, getters, rootState }) {
      switch (rootState.columnType) {
        ...
        case '1005':
          dispatch(
            'videoColumnDetailData/getVideoColumnDetail',
            { courseId: state.courseId },
            { root: true }
          )
          break
