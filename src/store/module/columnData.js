...
const columnData = {
    ...
    actions:{
        ...
        async getLessonList({commit,state},{courseId,refresh}){
            ...
            const res = await getLessonListByCourse({'courseId':courseId,currentPage:page, pageSize:state.pageSize})
            if(!res) return
            if(refresh){
                commit('saveStatus',
                {
        ...
        async getColumnDetail({ state, commit, dispatch }, { courseId, groupBuyId, columnType }) {
            ...
            //绑定专栏详情
            commit('saveStatus', {columnDetail,buyCount,userAccessStatus,renderLoading})
